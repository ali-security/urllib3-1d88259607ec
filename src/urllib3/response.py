    ...
    class BrotliDecoder(ContentDecoder):
        ...
        def decompress(self, data: bytes, max_length: int = -1) -> bytes:
            try:
                ...
                # Best effort: Brotli/brotlicffi/brotlipy older than 1.2.0 have
                # no `output_buffer_limit`, so the limit cannot be honoured and
                # this call decompresses without a bound. Kept silent on purpose
                # — the dependency floor is deliberately left at upstream 2.3.0's
                # value, so warning here would fire on every brotli response.
                return self._decompress(data)
